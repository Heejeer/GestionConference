...
def gerenate_user_id():
    return "USER"+uuid.uuid4().hex[:4].upper()

def verify_email(email):
    domaines=["esprit.tn","sesame.com","tek.tn","central.com"]
    email_domaine=email.split("@")[1]
    if email_domaine not in domaines :
        raise ValidationError("email doit appartenir à un domaine universitaire")
